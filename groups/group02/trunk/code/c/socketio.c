...
int recvall(int socket, byte *recvbuf, int bufsize, int flags) {
	...
	while (bytes_missing > 0) {
        ...
        if (bytes_received == 0) {
            DIE(TAG_SOCKETIO, "Sender disconnected");
        } else if (bytes_total > bufsize) {
            DIE(TAG_SOCKETIO, "Recvbuf overflow");
        }
	...
	LOG_DEBUG(TAG_SOCKETIO, "Packet received");
	return bytes_total;
}
...
int init_hs(int socket) {
	LOG_INFO(TAG_SOCKETIO, "Handshaking with %d", socket);
    ...
    LOG_DEBUG(TAG_SOCKETIO, "Handshake completed");
    return 0;

}

int wait_hs(int socket) {
	LOG_INFO(TAG_SOCKETIO, "Waiting for handshake from %d", socket);
	uint16_t client_shake = htons(DHT_CLIENT_SHAKE);
    uint16_t server_shake = htons(DHT_SERVER_SHAKE);
    uint16_t buf = 0;
...
int open_conn(int *sock, struct tcp_addr *addr) {
    LOG_INFO(TAG_SOCKETIO, "Opening connection");
    int status = 0;
    ...
    if ((status = getaddrinfo(addr->addr, addr->port, &hints, &info)) != 0) {
        DIE(TAG_SOCKETIO, "%s", gai_strerror(status));
    }
    if ((*sock = socket(info->ai_family, info->ai_socktype, info->ai_protocol)) == -1) {
        DIE(TAG_SOCKETIO, "Socket creation failed");
    }
    if ((status = connect(*sock, info->ai_addr, info->ai_addrlen)) == -1) {
        DIE(TAG_SOCKETIO, "Connecting socket failed");
        close(*sock);
    }
    freeaddrinfo(info);
    LOG_DEBUG(TAG_SOCKETIO, "Connection opened");
    return 0;
}
